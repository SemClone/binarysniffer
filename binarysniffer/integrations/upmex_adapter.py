...
class UPMEXAdapter:
    ...
    def extract_metadata(self, package_path: Path,
        ...
        try:
            if self._use_api:
                return self._extract_with_api(package_path, package_type)
            else:
    ...
    def _analyze_jar_basic(self, jar_path: Path) -> Dict[str, Any]:
        ...
        try:
            with zipfile.ZipFile(jar_path, 'r') as jar:
                ...
                pom_files = [f for f in jar.namelist() if f.endswith('pom.xml') or f.endswith('pom.properties')]
                if pom_files:
                    metadata['has_maven_metadata'] = True
                    metadata['maven_files'] = pom_files

                # Look for MANIFEST.MF
                manifest_files = [f for f in jar.namelist() if f.endswith('MANIFEST.MF')]
                if manifest_files:
                    try:
                        ...
                        for line in manifest_content.split('\n'):
                            if ':' in line:
                                ...
                                key = key.strip()
                                value = value.strip()
                                if key in ['Implementation-Title', 'Implementation-Version', 'Implementation-Vendor']:
                ...
                metadata['total_entries'] = len(jar.namelist())
